...
SST <- open_dataset("data/processed/all_scenarios_bias_corrected_temp_surface.parquet") # load projections
...
data_anomaly <- SST_future |> # calculate climate anomaly (future mean - historical mean / historical standard deviation)
  # filter(month == "12" | month == "1" | month == "2") |> # can filter by winter or summer months by changing month numbers
  summarize(average_future = mean(value), .by = c(lon_rho, lat_rho)) |>
  left_join(
    SST_hindcast |> summarize(average_hist = mean(value), sd_hist = sd(value), .by = c(lon_rho, lat_rho)),
